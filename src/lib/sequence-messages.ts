...
export type Message = { objet?: string; texte: string };

export const EMAIL_1 = (c: Contact): Message => ({
  objet: "Quick confirmation from Alex",
  texte: `Hey ${c.prenom}, looking forward to learning more about your business on ${jour(c)} at ${heure(c.debut, c.fuseau)}. ${phraseLien(c)}
...
export const EMAIL_3 = (c: Contact): Message => ({
  ...
  texte: `Hey ${c.prenom}, we're on in 30 minutes. ${phraseLien(c)}

Everything else is in one place here: the one-pager I put together, the results we've gotten for other owners, and how we did it. If you have a few minutes before we get on, take a look. Our conversation will be a lot more productive having seen it first.
